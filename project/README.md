...
- Install `torch` first when using `uv`. In this project, `torch==2.4.1+cpu` is pulled from the PyTorch CPU wheel index, and resolving it together with the rest of `requirements.txt` may fail because of index-selection rules.
- If you use a Python version other than 3.12, choose a Torch version that is compatible with your interpreter first, then run `uv pip install -r requirements.txt`.
- When in doubt, use the PyTorch official install selector to pick the correct command for your Python version and CPU / CUDA environment.
